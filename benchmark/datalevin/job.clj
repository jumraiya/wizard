...
(defn- load-table
  ...
  (let [db (with-open [rdr (io/reader (str file-dir "/" csv-file))]
...
(def conn (d/get-conn "/Users/jumraiya/projects/wizard/db"))
...
(def queries (into []
                   ...
                   (sort (keys (ns-publics 'benchmark.datalevin.job)))))
  ...
  (with-open [w (io/writer result-filename)]
    ...
    (doseq [q queries]
      (let [qname     (s/replace (name q) "q-" "")
            _         (print (str "  " qname "... "))
            query     (-> q (#(ns-resolve 'benchmark.datalevin.job %)) var-get)
...
(defn grid [&opts]
  (doseq [p [0.4]
    ...
    (let [start (System/currentTimeMillis)]
      (doseq [q queries]
        (let [query (-> q (#(ns-resolve 'benchmark.datalevin.job %)) var-get)]
