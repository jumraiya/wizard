...
(deftest simple-join-lmdb
  (let [ccircuit (c/build-circuit '[:find ?a ?b
    ...
    (is (match? #{[1 2 true]} output-1))
    (is (match? #{[1 2 false]
                  [3 4 true]} output-2))))

(deftest test-preds-lmdb
  (let [ccircuit (c/build-circuit '[:find ?a ?c
                                    :where
                                    [?a :attr-1 ?b]
                                    [(> ?b 4)]
                                    [(* ?b 100) ?c]])
        ...
        output (circuit c-state tx-data)]
    (is (match? #{[3 1000 true]} output))))

(deftest test-not-join
  (let [ccircuit (c/build-circuit '[:find ?a
                                    :where
                                    [?a :attr-1 ?b]
                                    (not-join [?b]
                                              [?b :attr-2 10])])
...
(deftest test-or-join-5
  (let [ccircuit (c/build-circuit '[:find ?a ?arg ?o ?det
                                    :where
                                    [?a :action/arg ?arg]
...
(deftest test-rules-free-vars
  (let [ccircuit (c/build-circuit '[:find ?c ?a
                                    :in $ %
                                    :where
                                    (rule ?a :const)
                   [?c :attr ?a]]
